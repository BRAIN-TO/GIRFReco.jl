...
*Figure 3. Reconstructed single-interleave spiral in-vivo human brain images (undersampling factor R=4). Top row: Images reconstructed from the nominal spiral gradient waveform ("No correction"), with correction for static off-resonance ("B0 map"), B0 + GIRF correction of the k-space trajectory ("GIRF k<sub>xyz"), and additional correction for GIRF k0 eddy currents ("Full Correction"). Bottom row: Consecutive absolute difference images of top-row reconstructions (5x scaled, i.e., +/- 20 % max image intensity). Cartesian image (from B0 map scan) is used as the reference; its edges are overlaid to assess geometric congruency of the spiral images.*
