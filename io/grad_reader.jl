...
#             interleaveGradArrayUpdated[:,k,1] = apply_girf(frequencyVec,GIRF[:,1], plannedTimes, interleaveGradArray[:,k,1],  plannedTimes)
#             interleaveGradArrayUpdated[:,k,2] = apply_girf(frequencyVec,GIRF[:,2], plannedTimes, interleaveGradArray[:,k,2],  plannedTimes)
...
#             k0_Array[:,k,1] = apply_girf(frequencyVecK0,GIRF_K0[:,1], plannedTimes, interleaveGradArrayUpdated[:,k,1],  plannedTimes)
#             k0_Array[:,k,2] = apply_girf(frequencyVecK0,GIRF_K0[:,2], plannedTimes, interleaveGradArrayUpdated[:,k,2],  plannedTimes)
...
##
function read_gradient_txt_file_interp_k(fileName, reconSize, delay)
    ...
    ## Convert to normalized k-space units (-0.5,0.5)
    convertedKSpaceArrayFlexible[:,:,1] *= dataDict[:FOV][1] ./ reconSize[1]
    convertedKSpaceArrayFlexible[:,:,2] *= dataDict[:FOV][2] ./ reconSize[2]
