...
tags:
  ...
  - Off-resonance Field Correction
authors:
...
Magnetic Resonance Imaging (MRI) acquires data under the frequency domain (k-space) using specific trajectory to traverse the whole k-space. The actual k-space trajectory during MRI scanning deviates from the theoretical one due to hardware imperfections, which induces image artifacts such as ghosting, blurring or geometric distortion, especially in non-Cartesian k-space trajectories such as spiral readouts [@lee_signalnoise_2021]. In this case, an accurate characterization of the system hardware for k-space trajectory correction, e.g. via a gradient impulse response function (GIRF) [@vannesjo_gradient_2013], is necessary for high-quality MRI reconstruction, which is especially essential for fast, SNR-efficient imaging protocols which place high demands on the hardware of the MRI system [@block_spiral_2005].
...
__GIRFReco.jl__ requires raw MR (k-space) data (in [ISMR]MRD format [@inati_ismrm_2017]) of the following scans as input:
...
2. Model-based correction components
    ...
    - Gradient impulse response function (GIRF) [@vannesjo_gradient_2013]
        - measured in phantom [@graedel_comparison_2017]
        - open-source computation [@wu_mr_nodate]
        - prediction implemented in custom package `MRIGradients.jl` [@jaffray_open-source_nodate]
